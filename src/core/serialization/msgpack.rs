...
impl MessagePackSerializer {
    /// Create new MessagePack serializer
    pub fn new() -> Self {
        Self {
            compact: true,
        }
...
impl Serializer for MessagePackSerializer {
    fn serialize<T: serde::Serialize>(&self, value: &T) -> Result<Vec<u8>, SerializationError> {
        rmp_serde::to_vec(value).map_err(|e| SerializationError::FormatError(e.to_string()))
    }
...
impl MessagePackDeserializer {
    /// Create new MessagePack deserializer
    pub fn new() -> Self {
        Self {
            strict: true,
        }
    ...
    /// Create new lenient MessagePack deserializer
...
impl Deserializer for MessagePackDeserializer {
    fn deserialize<T: serde::de::DeserializeOwned>(&self, bytes: &[u8]) -> Result<T, SerializationError> {
        rmp_serde::from_slice(bytes).map_err(|e| SerializationError::FormatError(e.to_string()))
    }
